...
export const useAttendance = () => {
  ...
  const buttonClass = computed(() =>
    isCheckedIn.value ? "btn-green" : "btn-red"
  );
  ...
  async function handleAttendance() {
    try {
      const response = await $fetch<{ message: string; toggleState: string }>(
      ...
      if (
        response.toggleState === "checkedIn" ||
        response.toggleState === "checkedOut"
      ) {
